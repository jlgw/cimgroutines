...
std::vector<int> fn(CImg<unsigned char> img, int min_radius,
    ...
    //This is slow for large radii, how can it be made more efficient?
    std::vector<int> outv;
    outv.reserve(STEP_CIRCLES);
    int radius_array[MAX_RADIUS]; //This is faster to search than multidim image
    int width = img.width();
    ...
    cimg_forXY(img, x1, y1){
        ...
        std::memset(radius_array, 0, (max_radius+1)*sizeof(int));
        ...
        im = indmax(&radius_array[min_radius], max_radius-min_radius+1)+min_radius;
    ...
    cimg_for5x5(vote, x1, y1,0,0,I,unsigned char){
        ...
        if((int)I.max()==(int)(vote(x1,y1))){
            //There should be a uniqueness check here for edge cases where we 
            //have two centers with identical number of votes.
